...
handler._user.put = (data, callback) => {
    ...
    if (email || password || fullname || address) {
        ...
        if (tokenID) {
            _data.readFile("tokens", tokenID, (err, data) => {
                // tokenPhone = data.phone;
            });
            if (email) {
                _data.isExist("users", email, exist => {
                    if (exist) {
                        callback(400, {err: `Cannot update the user email ${email} as it already exist`});
                    } else {
                ...
                //     address = address || userData.password;
                //     const newData = {
                //         email,
                //         password: helpers.hash(password),
                //         fullname,
                //         address,
                //     };
                //     _data.updateFile("users", newData.email, newData, err => {
                //         if (!err) {
                //             delete newData.password;
                //             callback(200, {msg: "successfully updated the user", err, data: newData});
                //         } else {
                //             callback(500, {msg: "Internal Server Error while updating file", err})
                //         }
                //     });
                // });
            }
        } else {
            callback(400, {msg: "Token required in header with  name 'token'"});
        }
    } else {
